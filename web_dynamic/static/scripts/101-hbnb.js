// JavaScript that toggles display of reviews when 'show' span is clicked
'use strict';
$(() => {
  ...
  const getReviews = placeId => {
    const reviewsFetcher = new Promise((resolve, reject) => {
      $.get(`${APIURL}/places/${placeId}/reviews`, (data, status) => {
        const reviewOwnerPromises = data.map(review => new Promise((resolve, reject) => {
          $.get(`${APIURL}/users/${review.user_id}`, (data, status) => {
    ...
    getPlaces(filter)
      .then(places => {
        ...
        for (let k = 0; k < places.length; k++) {
          $('section.places').append(createPlace(places[k]));
        }
      });
  };
  ...
  const reviewsToggleListener = evn => {
    const reviewsElement = evn.target.parentElement.parentElement;
    const reviewsULElement = reviewsElement.getElementsByTagName('ul').item(0);

    if (evn.target.innerText === 'show') {
      const placeId = evn.target.getAttribute('data-id');

      reviewsULElement.innerHTML = '';
      getReviews(placeId)
        .then(reviews => {
          for (let k = 0; k < reviews.length; k++) {
            const reviewElement = createReview(reviews[k]);

            reviewsULElement.insertAdjacentElement('beforeend', reviewElement);
          }
          evn.target.innerText = 'hide';
        });
    } else if (evn.target.innerText === 'hide') {
      reviewsULElement.innerHTML = '';
      evn.target.innerText = 'show';
    }
  };
